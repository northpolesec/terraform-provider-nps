---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "nps_workshop_rule_pack_subscription List Resource - nps"
subcategory: ""
description: |-
  List all rule pack subscriptions in the Workshop instance.
---

# nps_workshop_rule_pack_subscription (List Resource)

List all rule pack subscriptions in the Workshop instance.

## Example Usage

```terraform
# Query files use the .tfquery.hcl extension and are run with `terraform query`.
# This block lists every rule pack subscription.
list "nps_workshop_rule_pack_subscription" "all" {
  provider = nps

  # Terraform returns 100 results per list block by default. Raise `limit` to
  # cover a larger instance.
  limit = 1000

  # Return full resource objects rather than just identities. Useful when you
  # intend to generate import blocks from the results.
  include_resource = true
}
```

<!-- schema generated by tfplugindocs -->
## Schema
